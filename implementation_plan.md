...
The following 10 features directly extract the core innovation from the Notion research document, prioritized for maximum hackathon impact and total technical differentiation from generic tracking apps.
...
* **What it does**: In total dead zones (no cell towers, no internet), an SOS alert generated by Tourist A hops wirelessly across nearby tourists' phones (Tourist B $\rightarrow$ Tourist C) until reaching a device that has network access to forward the alert to the backend.
...
```
+---------------------------------------------------+
|  AEGIS SAFEPASS                   [🟢 Mesh Active] |
|---------------------------------------------------|
|                                                   |
|   +-------------------------------------------+   |
|   | 🛡️ TOURIST DIGITAL ID                     |   |
|   | ID: AEGIS-8F29X4   [VALID: 4 DAYS LEFT]   |   |
...
|   +-------------------------------------------+   |
|   | 📍 CURRENT ZONE STATUS                    |   |
|   | 🟡 CAUTION: Cherrapunji Ridge              |   |
|   | Dynamic Hazard: Heavy Rainfall Warning    |   |
|   | GPS: 25.141, 91.261 | Satellites: 9 Lock |   |
|   +-------------------------------------------+   |
|                                                   |
|   +-------------------------------------------+   |
|   |             🚨 PANIC / SOS 🚨             |   |
...
|   +-------------------------------------------+   |
|                                                   |
|  [🏠 Home]    [🗺️ Offline Map]    [⚠️ Report Hazard] |
+---------------------------------------------------+
```
...
```
+-----------------------------------------------------------------------------------+
| 🛡️ AEGIS COMMAND CENTER | Live Incidents: 2 | High Risk: 5 | Responders Active: 14 |
+-----------------------------------------------------------------------------------+
| [ MAP CANVAS - OPENSTREETMAP + MAPLIBRE ]                      | INCIDENT DRAWER   |
|                                                                |-------------------|
|   🟢 TST-829 (Safe)       🟡 TST-412 (Route Dev)               | 🚨 CRITICAL SOS   |
|                                                                | ID: TST-9912      |
|           🔴 [PULSATING SOS: TST-9912]                         | Loc: 25.145,91.265|
|           Hazard Zone: Flood Region B                         | Batt: 14% | 14:32 |
|                                                                | Risk Score: 100/100|
|   ----------------------------------------------------------   | Responders Nearby:|
|   [Overlay: Weather Radar] [Overlay: Geofences] [Risk Heatmap] | 1. Rescue Team 2  |
|                                                                |    (3.2 km - 8m)  |
|                                                                | [ DISPATCH UNIT ] |
+-----------------------------------------------------------------------------------+
```
...
## 📅 Phased Strategy & 24-Hour Hackathon Execution Roadmap

```mermaid
gantt
    title AEGIS 24-Hour Sprint Roadmap
    dateFormat  HH:mm
    axisFormat %H:%M

    section Phase 1: Foundation (Hours 0-4)
    Solidity Smart Contract (Sepolia)   :active, h1, 00:00, 3h
    Android Native Project Init (`android create`) :h2, 01:00, 3h
    Node.js Backend & WebSocket Gateway :h3, 02:00, 4h

    section Phase 2: Core Features (Hours 4-12)
    Android Room Offline DB & Geofence Engine :h4, 04:00, 4h
    Digital Tourist ID & QR Code Engine      :h5, 06:00, 4h
    SOS Dual-Channel Dispatch (WS + Compact Payload) :h6, 08:00, 4h

    section Phase 3: Mesh & Dashboard (Hours 12-18)
    P2P BLE / Wi-Fi Direct Mesh Relay      :h7, 12:00, 4h
    Authority Command Center Web UI         :h8, 14:00, 4h
    AI Multi-Factor Risk Scoring Engine     :h9, 16:00, 3h

    section Phase 4: Polish & Demo Prep (Hours 18-24)
    Crowdsourced Hazard Reporting & Sync   :h10, 18:00, 3h
    End-to-End System Integration Test      :h11, 20:00, 3h
    Demo Simulation & Pitch Assets          :h12, 22:00, 2h
```
...
> [!IMPORTANT]
> **Plan Approval**:
> All design rules, zero-cost stack choices, the **AEGIS** project name, real Solidity smart contract target, and top 10 detailed feature specifications are finalized in this plan.
> 
> Once you confirm and approve this implementation plan, we will move directly to creating the codebase!
