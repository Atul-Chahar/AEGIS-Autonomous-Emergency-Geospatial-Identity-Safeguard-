...
1. **AEGIS Identity Architecture & Smart Contract Integration (`aegis-contracts` & `aegis-backend`)**:
...
2. **AEGIS Hazard Confidence Engine & Sybil Protection (`aegis-backend`)**:
...
3. **AEGIS Backend Modular Architecture & PostGIS Persistence (`aegis-backend`)**:
...
4. **AEGIS Offline Peer Relay (Google Nearby Connections)**:
...
5. **AEGIS Outbox Pattern & Backend Connection**:
...
6. **AEGIS Offline Geospatial Safety Engine**:
...
7. **AEGIS Sensor Fusion Risk Engine**:
...
8. **AEGIS BlackBox (Offline Breadcrumb & Sensor Logging)**:
