...
- **Unique per-install identity is real**: `LocalIdentityRepository` generates a unique pseudonymous `TST-XXXXXX` ID on first launch (persisted), and registers its `keccak256(touristId:salt)` commitment with the gateway on startup — so every device shows a distinct ID on the dashboard and incidents link a real `idHash` (backend `IncidentService` attaches the tourist's hash; verified `0xfac5…` linked).
- **Dashboard map covers the full sector**: map bounds now span Cherrapunji **and** Dawki (lon 91.15–91.95), so SOS incidents/relays at Dawki are visible; `MapController` fits all subjects when none is selected. Dashboard cards/drawer show honest `--`/`Not specified` instead of fabricated route/zone/battery/accuracy fallbacks.
...
- **`DemoIdentityRepository`**: used only inside `@Preview` composables (e.g. `HomeScreenPreview`) as isolated preview defaults — never in the real navigation graph. The production identity source is `LocalIdentityRepository` (unique per-install ID + gateway registration).
- **`DemoSafetyZoneRepository`**: likewise preview-only.
